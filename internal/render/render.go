...
func Build(opts Options) error {
	...
	for _, kind := range content.AllKinds {
		...
		for _, path := range files {
			...
			if kind == content.KindCollection {
				meta.Collection, err = renderCollection(opts.ContentDir, doc)
				if err != nil {
					return err
				}
			}
...
type pageData struct {
	...
	TOC template.HTML
	// Collection is the rendered sorted views on a collection page. It is
	// placed after the markdown body, so the page's prose introduces it.
...
# The books list moved twice: from a generated dashboard to a list article, and
# then to a data-driven collection page. Both old paths are kept permanently so
# existing links and bookmarks do not break.
/books/ /collections/books/ 301
/books /collections/books/ 301
/lists/books/ /collections/books/ 301
/lists/books /collections/books/ 301
`
...
const homeIntro template.HTML = `<p>I'm Gordy Clark. I build software at <a href="https://workmind.cloud">Workmind</a>, but this is my ` +
...
var templateFuncs = template.FuncMap{
	...
	// booksURL is the nav's link to the books collection. It is derived from
	// the kind rather than written out, so the site's rule that URLs come from
	// content.Kind holds in the nav too.
	"booksURL": func() string { return content.KindCollection.URL(booksSlug) },
}

// booksSlug is the slug of the books collection, which the nav links directly
// rather than sending readers through a section index that lists one page.
const booksSlug = "books"
